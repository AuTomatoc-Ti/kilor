...
Spatial postpositions occupy the same slot as `tilpe` in the oblique PP order. When multiple spatial postpositions co-occur, they follow the order: `ikne` / `oukne` > `paune` / `haune` > `hinne` > `rapne` / `umne` > `orane` > `meipone` > `tilpe` > `tene`.
